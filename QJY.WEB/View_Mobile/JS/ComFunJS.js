...
var ComFunJS = {
    ...
    viewimg: function (tpdata, isDownload) {
        ...
        $(tpdata).each(function (index, ele) {
            ...
            if (!isDownload) {
                ...
                html = '<li itemid="' + ele.ID + '" class="weui_uploader_file mall_pcp" >';
                html += '<a href="javascript:ComFunJS.getfile(' + ele.ID + ')">';
                html += '<img class="ComFunJS.isPic(' + ele.FileExtendName + ')" class="" onclick="javascript:ComFunJS.getfile(\'' + ele.YLUrl + '\')" src="/ViewV5/images/qywd/' + ele.FileExtendName + '.png"/>';
                html += '<span class="fileinfo" ms-if="!ComFunJS.isPic(file.FileExtendName)">' + ele.Name + '</span>';
                html += '</a></li>';
            }
            $html1 = $(html);
            $html1.appendTo($("#imglist1"));
    ...
    showchild: function (str) {
        if (str) {
            if ($(str).children('.list_child_block').css('display') == 'none') {
                $(str).children('.list_child_block').show();
            }
            else {
                $(str).children('.list_child_block').hide();
            }
        }
    },
                ...
                for (i = 0; i < rs.length; i++) {
                    for (n = 0; n < ComFunJS.facePath.length; n++) {
                        if (ComFunJS.facePath[n].faceName == rs[i].substring(1, rs[i].length - 1)) {
                            var t = "<img title=\"" + ComFunJS.facePath[n].faceName + "\" style='display: initial;width: auto !important;' src=\"/View_Mobile/Images/face/" + ComFunJS.facePath[n].facePath + "\" />";
